...
const Game = props => (
    <div className="game">
      {props.gameStatus !== 'active'
          ? <PlayAgain onClick={props.onClick} gameStatus={props.gameStatus}/>
...
const useGameState = () => {
  ...
  useEffect( () => {
    ...
    return () => console.log('Component is changing -> Going to rerender');
  });
...
const GameMaster = (props) => {
  ...
  const onNumberClick = (number, currentStatus) => {
    ...
    if (currentStatus === 'wrong' || currentStatus === 'candidate') {
...
const StarMatch = () => {
  ...
  return <GameMaster timeLimt={15} key={gameId} startNewGame={() => setGameId(gameId +1)}/>
}
// Color Theme
const colors = {
  available: 'lightgray',
